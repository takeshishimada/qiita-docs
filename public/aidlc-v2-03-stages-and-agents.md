...
`mob` と、補佐を宣言した `subagent` では、起動された補佐が自分の書いたものを自分のファイルに残し、成果物を編集するのは主担当だけになります。このファイルは完了の証拠にもなります。`pipeline` では鎖の各段が成果物を直接書き換えるので、この記録は要りません。4つのかたちの詳細、異議が割れたときの仕分け、並行できない環境での約束は、別記事「協働のトポロジー」で扱います。
