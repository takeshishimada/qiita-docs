...
代わりに、1体のエージェントが複数のステージ・複数のフェーズを横断します。たとえばアーキテクトエージェントは、実現性評価・アプリケーション設計・作業単位の生成・機能設計・NFR 要件（非機能要件）・NFR 設計と、3フェーズにまたがる6ステージを担当します。同じエージェントが連続して受け持つため、引き継ぎが要らず、情報がそのまま残ります。人のチームでも、少人数の混成チームが1つの機能をまとめて担当するほうが、大人数のリレーより情報がこぼれません。それと同じ発想です。

### 主担当と補佐

各ステージには**主担当**（Lead）が置かれ、必要に応じて**補佐**（Support）が付きます（32ステージ中12は補佐なし）。初期化の3ステージだけは、主担当もエージェントではなくコンダクター自身です。主担当がそのステージの成果物に責任を持ち、補佐は専門知識を持ち込みます。たとえば NFR 要件のステージはアーキテクトが主担当ですが、セキュリティ観点で DevSecOps エージェントが、規制観点でコンプライアンスエージェントが補佐に入ります。
...
出荷時点では **32ステージのうち28が `inline`** で、支配的なのは「コンダクターが全員の役を引き受ける」形です。残る4つは、ステージ本体そのものを複数のエージェントで回します。リバースエンジニアリングが `pipeline`、ユーザーストーリーが `mob`、作法の発見とコード生成が `subagent` です。
...
| エージェント | ドメイン | 主担当ステージ | 主な活動フェーズ（補佐も含む） |
| --- | --- | --- | --- |
| プロダクト | 要件・ユーザーストーリー・スコープ | 意図キャプチャ、市場調査、スコープ定義、要件分析、ユーザーストーリー | 発想・構想 |
| デザイン | UX/UI・ワイヤーフレーム | ラフ／詳細モックアップ | 発想・構想 |
| デリバリー | チーム編成・デリバリー順序 | チーム編成、承認・引き継ぎ、デリバリー計画 | 発想・構想 |
| アーキテクト | アプリ設計・ドメイン設計・NFR | 実現性評価、アプリケーション設計、作業単位の生成、機能設計、NFR | 発想・構想・構築 |
| AWSプラットフォーム | インフラ・CDK・コスト最適化 | インフラ設計、環境プロビジョニング | 発想・構想・構築・運用 |
...
| 品質 | テスト戦略・テスト生成・性能検証 | ビルド・テスト、性能検証 | 構想・構築・運用 |
| パイプラインデプロイ | CI/CD・デプロイ戦略・リリース | 作法の発見、CIパイプライン、デプロイパイプライン、デプロイ実行 | 構想・構築・運用 |
| オペレーション | 可観測性・インシデント対応・SLO | 可観測性セットアップ、インシデント対応、フィードバック・最適化 | 運用 |
...
- 実行するのは、担当範囲を広く取った14体のエージェント（成果物を作る11体＋レビュー専任2体＋スコープを組むコンポーザー1体）。主担当と補佐で分担し、その協働のかたちは `mode` が決める（28ステージは `inline`、残る4つはステージ本体を複数のエージェントで回す）。連携は常にコンダクター経由で、レビュアーは止めずに品質判定を添えるだけ
