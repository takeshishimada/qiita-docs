...
AI-DLC v2 のワークフローは、開発の狙いで区切ったフェーズと、その下に並ぶ全32ステージで組み立てられます。32は全量で、実際に通る数は案件により絞られます（詳しくは別記事「[スコープ](https://qiita.com/takeshishimada/items/c232fb2e994e7b567a5c)」で扱います）。そして各ステージを実際に動かすのは、成果物を作る11体の専門エージェントと、できあがった成果物をレビューする2体、それにスコープを組むコンポーザー1体を加えた14体です。一つひとつのエージェントは担当範囲を広く取り、フェーズをまたいで複数のステージを受け持ちます。
...
5つのフェーズは、**初期化 → 発想 → 構想 → 構築 → 運用** の順に並び、原則として順番に実行されます。初期化から発想への移行を除く各境目では、**フェーズ境界検証**が走ります。成果物の欠落やトレーサビリティの切れ目を点検し、進行の記録（監査ログ）に `PHASE_VERIFIED` という印を残す仕組みです。これは自動で進行を止めるものではなく、問題があれば承認ゲートで人が判断します。検証の中身は別記事「[フェーズ境界検証](https://qiita.com/takeshishimada/items/f2f4e426dd542c5b6765)」で扱います。
...
もう1つの軸はエージェントです。AI-DLC v2 のステージを動かすのは、**成果物を作る11体の専門エージェント**と、**レビューだけをする2体のレビュアーエージェント**、それに**スコープを組む1体のコンポーザー**（計14体）です。

### 担当範囲を広く取る理由

AI-DLC v2 は、担当範囲の狭い専門家を何十体も並べる方式をあえて取っていません。**エージェント同士の境界は、情報が失われるポイントだから**です。担当が変わるたびに引き継ぎ文書が要り、その受け渡しのたびに情報が抜け落ちます。

代わりに、1体のエージェントが複数のステージ・複数のフェーズを横断します。たとえばアーキテクトエージェントは、実現性評価・アプリケーション設計・作業単位の生成・機能設計・NFR 要件（非機能要件）・NFR 設計と、3フェーズにまたがる6ステージを担当します。同じエージェントが連続して受け持つため、引き継ぎが要らず、情報がそのまま残ります。人のチームでも、3〜5人で組んで1つの機能をまとめて担当するほうが、大人数のリレーより情報がこぼれません。それと同じ発想です。

### 主担当と補佐

各ステージには、**主担当**（Lead）と**補佐**（Support）が割り当てられます。主担当がそのステージの成果物に責任を持ち、補佐は専門知識を持ち込みます。たとえば NFR 要件のステージはアーキテクトが主担当ですが、セキュリティ観点で DevSecOps エージェントが、規制観点でコンプライアンスエージェントが補佐に入ります。

そして、エージェント同士は直接呼び合いません。委譲はすべて、進行役であるコンダクター（エンジンが決めた手を実行する側）を通します。だから「誰が誰を呼んだか」が常に一本のフラットな線で追え、呼び出しが再帰的に深くなることもありません。

### 協働のかたち

主担当と補佐が**どう協働するか**は、ステージごとに違います。それを決めるのがフロントマターの `mode` で、「ステージ本体を走らせるあいだ、誰が誰と話すか」を指します。4種あります。

| mode | 協働のかたち | 出荷数 |
| --- | --- | :-: |
| `inline` | コンダクターが全員の役を自分で引き受ける。補佐は独立に動かず、観点として持ち込まれる | 28 |
| `subagent` | ハブとスポーク。主担当が下書きを書き、各補佐が**互いを見ない状態で**それを検分し、主担当が統合する | 2 |
| `pipeline` | 鎖。補佐が宣言された順に手を入れ、各段が上流の作業をすべて見る。最後の段が仕上げる | 1 |
| `mob` | 一つの部屋。全員が主担当の下書きに並行して意見を出し、残った異議は記録される | 1 |

出荷時点では **32ステージのうち28が `inline`** で、支配的なのは「コンダクターが全員の役を引き受ける」形です。残る4つが実際に別のエージェントを起動します。リバースエンジニアリングが `pipeline`、ユーザーストーリーが `mob`、作法の発見とコード生成が `subagent` です。

`mob` と、補佐を宣言した `subagent` では、起動された補佐が自分の書いたものを自分のファイルに残し、成果物を編集するのは主担当だけになります。このファイルは完了の証拠にもなります。`pipeline` では鎖の各段が成果物を直接書き換えるので、この記録は要りません。4つのかたちの詳細、異議が割れたときの仕分け、並行できない環境での約束は、別記事「[協働のトポロジー](https://qiita.com/takeshishimada/items/47e11209aed038250b7a)」で扱います。
...
| アーキテクト | アプリ設計・ドメイン設計・NFR | 実現性評価、アプリ設計、作業単位の生成、機能設計、NFR | 発想・構想・構築 |
...
この2体は一部のステージで、成果物の完成後・承認ゲートの前に独立評価を添えますが、ワークフローを止める権限は持ちません。最終判断は承認ゲートの人に委ねます。成果物を作る11体と同じくコンダクターが起動し、レビュアー同士も呼び合いません。判定の中身は別記事「[レビュアー](https://qiita.com/takeshishimada/items/624d83e946e86e4b1553)」で扱います。
...
- 実行するのは、担当範囲を広く取った14体のエージェント（成果物を作る11体＋レビュー専任2体＋スコープを組むコンポーザー1体）。主担当と補佐で分担し、その協働のかたちは `mode` が決める（28ステージは `inline`、残る4つが別のエージェントを起動する）。連携は常にコンダクター経由で、レビュアーは止めずに品質判定を添えるだけ
...
| [`aidlc-common/protocols/stage-protocol.md`](https://github.com/awslabs/aidlc-workflows/blob/9f91454/core/aidlc-common/protocols/stage-protocol.md) | 主担当（Lead）・補佐（Support）の割り当て、フェーズ境界検証、承認ゲート |
...
| [`agents/aidlc-architecture-reviewer-agent.md`](https://github.com/awslabs/aidlc-workflows/blob/9f91454/core/agents/aidlc-architecture-reviewer-agent.md)／[`aidlc-product-lead-agent.md`](https://github.com/awslabs/aidlc-workflows/blob/9f91454/core/agents/aidlc-product-lead-agent.md) | レビュー専任の2体 |
| [`aidlc-common/conductor.md`](https://github.com/awslabs/aidlc-workflows/blob/9f91454/core/aidlc-common/conductor.md) | 委譲（`Task`）はコンダクター専用・エージェントは互いを呼ばない |
